...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/05/29 12:45:29 by atlekbai          #+#    #+#             */
/*   Updated: 2018/05/29 12:45:30 by atlekbai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			frk(t_byte **map, t_proc **procs, t_game *game)
{
	...
	index = ((*procs)->pc + ((int)arg % IDX_MOD)) % MEM_SIZE;
	if (index < 0)
		index = MEM_SIZE + index;
	(*game).procs_arr = add_proc((*game).procs_arr, index % MEM_SIZE,
			++(*game).procs);
	arg = -1;
	while (++arg < 16)
